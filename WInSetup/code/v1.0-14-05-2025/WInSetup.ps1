...
function Check-AppInstalled {
    ...
    # Check winget for Store apps
    try {
        $wingetList = & winget list 2>$null
        switch ($AppName) {
            "WhatsApp" { if ($wingetList -match "9NKSQGP7F2NH") { return $true } }
            "Telegram" { if ($wingetList -match "9NZTWSQNTD0S") { return $true } }
            "Netflix" { if ($wingetList -match "9WZDNCRFJ3TJ") { return $true } }
            "Instagram" { if ($wingetList -match "9NBLGGH5L9XT") { return $true } }
            "Discord" { if ($wingetList -match "XPDC2RH70K22MN") { return $true } }
            "Spotify" { if ($wingetList -match "9NCBCSZSJRSB") { return $true } }
            "iTunes" { if ($wingetList -match "9PB2MZ1ZMB1S") { return $true } }
        }
    } catch {
...
$internetURLs = @{
    ...
    "OBS" = "https://cdn-fastly.obsproject.com/downloads/OBS-Studio-30.0.0-Full-Installer-x64.exe"
    "Medal" = "https://cdn.medal.tv/downloads/MedalSetup.exe"
    ...
    "FileZilla" = "https://download.filezilla-project.org/client/FileZilla_3.66.1_win64-setup.exe"
    "FXSound" = "https://downloads.fxsound.com/fxsoundsetup.exe"
...
foreach ($app in $internetApps) {
    ...
    $btn.Add_Click({
        ...
        try {
            ...
            $this.Text = "$appName (Installer Started)"
        } catch {
